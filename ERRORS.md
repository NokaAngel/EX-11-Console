...
## LED Power-On Flash Codes (POC)

The board runs internal self-tests at power-up (Power-On Checks). If one fails, it
flashes the status LED a set number of times, pauses, and repeats. **This is separate
from the DX sensor test** - a flashing LED means the *controller itself* flagged a fault,
not just the sensor.

### Official 3M EX II Flash Code Table

From 3M's **EX II Serial Controllers Reference Guide (TSD-29087, Table 2)**. These codes are definitive, not reconstructed.

| Flashes | Official 3M Code | Meaning | Remedy |
|---------|---|---|---|
| 0 | - | No errors (normal) | Steady dim LED - operating correctly |
| 1-2 | Reserved | Reserved codes | Count again between pauses |
| 3 | Setup error | Could not establish operating range at power-up | Nonrecoverable - replace controller (might also be touch screen) |
| 4 | Controller NOVRAM error | Operating parameters invalid; running on defaults | Run `recover` (Restore Defaults); if persists, replace controller (might be screen/cable) |
| 5 | Hardware (HDW) error | Hardware failed - could not initialize or load program | Nonrecoverable - replace controller |
| 6 | Reserved | Reserved code | Count again between pauses |
| 7 | Cable NOVRAM error | Linearization data in cable NOVRAM is invalid | Flashes until controller receives a valid command (tool sends one, so may stop when connected). Re-linearize or replace cable. |
| 8 | Controller linearization error | Controller linearization data in NOVRAM is invalid | Replace touch screen or perform 25-point linearization; contact 3M support |
| 9 | Reserved | Reserved code | Count again between pauses |
| 10 | EEPROM not formatted | Controller EEPROM not formatted | Reload program code |
| 11 | Invalid block 5 | (not applicable to serial controllers) | N/A |
| 12 | Invalid block 6 | Invalid block 6 | Restore Defaults if possible, otherwise replace controller |

### Key Points

- **Codes 1-2, 6, 9, 11:** Reserved or not applicable to serial controllers - if you count these, recount between the pauses.
- **Code 4 is recoverable** with `recover` (Restore Defaults); **Code 5 is not** (nonrecoverable hardware failure).
- **Code 7 (cable NOVRAM)** flashes until the host sends any command - your tool sends one, so it may stop once you connect.

### Recovery procedure

3M's documented escalation, in order:

1. **Restore Defaults** - in the tool, run `recover` (sends Reset, Restore Defaults, Reset, then re-tests DX).
2. **Power-cycle** the board fully and recount the flashes.
3. If the same code returns after both, the fault is in **hardware** (NOVRAM/analog) and the board is bad.

### Using the tool

```
ex11> flash 5      # decode the flash code you counted
ex11> recover      # run the Restore Defaults recovery sequence
ex11> report log.txt   # save a full snapshot (identity + DX) to a file
```

### Why Flash Codes Matter for Stuck DX=2

A controller power-on fault (especially codes 3, 4, 5: setup, NOVRAM, hardware) prevents normal touch operation because the board failed **its own internal tests** before the sensor was ever involved. If you see:

- **A flashing LED at power-up** (codes 3, 5, 8, 10, 12)
- **Plus a persistent DX=2** (sensor self-test failure)
- **Even with a known-good capacitive panel**

Then the flash code is your **root cause**, not the sensor. The board's internal fault cascades into `DX=2` because it can't properly initialize the sensor circuits.

**Example:** Code 5 (hardware failure) means the ASIC couldn't load its program. It won't be able to read a sensor no matter how good the panel is. Replace the controller.

**Counter-example:** Code 4 (NOVRAM config) means settings are corrupted - run `recover` first, since this is fixable without a new board.
